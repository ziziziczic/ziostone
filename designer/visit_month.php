...
<tr>
	...
	<table border='0' cellpadding='5' cellspacing='0' width='100%'>
		<tr>
			<td height='30'><font color='#CC0000'><b>월별 방문자 현황</b></font></td>
		</tr>
	</table>
	</td>
</tr>
	...
	<tr>
		<td width='100%'>
			<table width=100% cellpadding=4 cellspacing=1 border=0 bgcolor=ECE9D8>
				<tr bgcolor=F8F5F0>
					<td rowspan=2 width=80 align=center>년-월</td>
...
do {
    ...
    $query = " select vi_time, vi_user_agent from $DB_TABLES[visit] where substring(vi_date,1,7)='$job_month' ";
    ...
    <tr align=center class=$class>
        <td width=80><a href='visit_date.php?fr_date=$job_month-01&to_date=$job_month-31'>$job_month</a></td>
        <td>$brower_msie40</td>
        <td>$brower_msie50</td>
        <td>$brower_msie55</td>
        <td>$brower_msie60</td>
    ...
    $k++;
    $next_month  = mktime (0,0,0, substr($job_month,5,2)+1 , 01, substr($job_month,0,4));
    $job_month=date("Y-m", $next_month);
} while($job_month <= $to_month);
...
for ($i=0; $i<$exp_num; $i++) {
	...
	$query = "select count(vi_id) from $DB_TABLES[visit] where ({$referer_url}) and substring(vi_date,1,7) between '$fr_month' and '$to_month'";
...
");
    ...
    $query = " select vi_referer, count(vi_referer) as vi_referer_count
             ...
             where substring(vi_date,1,7) between '$fr_month' and '$to_month'
             group by vi_referer
