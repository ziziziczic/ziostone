...
function TreeMenuItem(text, url, target, icon, pop, del, winsize) {
...
function relWinSize()
{
	parent.window.resizeTo (1000, 1000);
}
...
function treeMenuDisplay() {
  ...
  treeMenuDocument.writeln('<style type="text/css">a {FONT-SIZE: 9pt; text-decoration:none;}</style>');
  treeMenuDocument.writeln('</head>');
...
//  treeMenuDocument.writeln('<base target="mainFrame">');
  ...
  treeMenuDocument.write('<font face="' + treeMenuFont + '">&nbsp;' + treeMenuRoot + '&nbsp;</font>');
...
function treeMenuDisplayItem(item) {
  ...
  var bars, cmd, expanded, i, img, alt, link, more, submenu, ppp, pop;
  ...
  ppp = "return parent.OpenWindow(" + item.pop + ");";
  ...
  if (item.url != "") { // URL 이 있있고
	  if(item.pop == "") { // 팝업창이 없을때 타켓으로 이동
				treeMenuDocument.write('<a href="' + item.url + '" target="' + item.target + '" onClick="' + cmd + '">'); 
	} else {  

		if(item.url =="#") {
		// 팝업창이 있을때 새창열기
		    treeMenuDocument.write('<a href="' + item.url + '" onClick="' + ppp + '">'); 
		} else { // 새창이 열리면서 오른쪽 화면에 도움말 표시 url이 오른족 화명에 보여진다.
		    treeMenuDocument.write('<a href="' + item.url + '" target="' + item.target + '" onClick="' + ppp + '">'); 
		}
	}
  }
  else{ // URL이 없을때 트리자체 이동
	  if(item.del != "") {  // 삭제 기능 쓸때..
   treeMenuDocument.write('<a href="' + item.del + '">'); 
	  } else {
  ...
  if (item.index == treeMenuSelected)
    treeMenuDocument.write('<font face="' + treeMenuFont + '">&nbsp;<span style="background-color:' + treeMenuHiBg + ';color:' + treeMenuHiFg + ';">' + item.text + '</span></font>');
  else
    treeMenuDocument.write('<font face="' + treeMenuFont + '">&nbsp;' + item.text + '</font>');
