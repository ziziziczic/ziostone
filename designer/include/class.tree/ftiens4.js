...
function openWindow(name, url, left, top, width, height, toolbar, menubar, statusbar, scrollbar, resizable) // 새창열기
{
...
function gLnk(target, description, linkData, linkValue, target_num, openname, openmethod) 
{ 
	if(target_num == 1) {
		var movetarget = "target = 'menuFrame'";
	} else if(target_num == 2) {
		var movetarget = "target = 'mainFrame'";
	} else if(target_num == 3) {
		var movetarget = "target = 'new'";
	} else { 
		var movetarget = "";
	}
  ...
  { 
    if (target==1)  {
       fullLink = "'http://"+linkData+"' target=_blank" 
		...
		if(target_num == "3") {
		  fullLink = "\"javascript:openWindow('" + openname + "' , '" + linkData + "?" + linkValue + "' , " + openmethod + ")\"" 
		} else { 
	      fullLink = "'" + linkData + "?" + linkValue + "'" + movetarget + "\""
		}
