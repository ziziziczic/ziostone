...
/*
	// 설정을 연관배열로 얻어오는 함수
	function get_setup_info($info, $info_sep, $item_sep) {
		$setup_info = array(); // 리턴값을 저장할 배열변수;
		$info_exp = explode($info_sep, $info);	// 각 설정을 나눈다.
		for ($i=0; $i<sizeof($info_exp); $i++) {	 // 저장되어 있는 설정의 개수만큼 반복
			$item_exp = explode($item_sep, $info_exp[$i]);	 // 설정 이름과 값을 분리하여 연관배열로 저장한다.
			$setup_info[$item_exp[0]] = $item_exp[1];
		}
		return $setup_info;
	}
*/
	...
	*/
	...
	function get_page_block($query, $ppa, $ppb, $page, $style, $font, $img_dir='', $link_file='', $unlink_view='N', $var_name_page='page', $total_type='C', $change_vars=array()) {
		...
		*/
		...
		$return_value = array($page_link_list, $ppb_value, $T_page);
		return $return_value;
	}
...
//	 인터페이스 관련	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function get_radio_array($list_value, $name, $default_value, $property='', $divider='') {
		...
		$input_type = substr($input_type, 0, -strlen($divider));
		return $input_type;
	}

	// 다중 체크상자를 만드는 함수 <checkbox>
	function get_checkbox_array($list_value, $name, $default_value, $property='', $divider='') {
		...
		for ($i=0; $i<sizeof($option_value); $i++) {
			...
			$input_type .= "<input type='checkbox' name='$new_name' value='$trimed_value' onclick=\"multi_check(this.form, '" . substr($new_name, 0, -2) . "', '$name', '$multi_sep')\"{$property}{$T_is_checked}>{$trimed_name}{$divider}";
		}
		if ($divider != '') $input_type = substr($input_type, 0, -strlen($divider));
		if ($default_value != '') $O_default_value = implode($multi_sep, $default_value);
		$input_type .= "<input type='hidden' name='$name' value='$O_default_value'>";
	...
	function get_list_boxs_array($list_value, $name, $default_value, $exist_null='Y', $property='', $default_num_msg=":: 선택 ::") {
		...
		if (eregi("multiple", $property)) {
			$new_name = $name . "_multi";
			if ($default_value != '') $O_default_value = implode($multi_sep, $default_value);
			$etc_tag = "<input type='hidden' name='$name' value='$O_default_value'>";
		...
		return $this->make_list_box($new_name, $option_name, $option_value, '', $default_value, $property, '') . $etc_tag;
	}
	...
	function make_input_box($value, $name, $type, $property, $style, $default_value='Y', $is_strip_slashes='Y') {
		...
		switch ($type) {
			...
			break;
			case 'radio' :
				if ($value == $default_value && $value != '' && !eregi("disabled", $property)) $property .= " checked";
