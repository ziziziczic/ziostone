...
while ($value = mysql_fetch_array($result)) {
	...
	$query = "insert into MAMUL_BACKUP values(0, '$value[fid]', '$value[name]', '$value[email]', '', '$value[homepage]', '$value[subject]', '$value[comment]', '$value[comment2]', '', '$value[signdate]', '$value[count]', '$value[thread]', '', '', '', '', '', '$value[category]', '수도권', '일반', '', '', '', '', 'Y', '', '', '', '', '')";
	$insert_result = @mysql_query($query) or die("타겟 매물 입력 쿼리중 에러<br>" . mysql_error());
}

// 지방권
$source_table = "TCBOARD_MAIN_5N";
$query = "select * from $source_table";
