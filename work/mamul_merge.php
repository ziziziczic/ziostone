...
while (list($key, $key_value) = each($source_table_name)) {
	...
	while($value = mysql_fetch_array($result)) {	
		...
		$query = "insert into MAMUL_BACKUP values(0, '0', '$value[name]', '$value[email]', '', '$value[homepage]', '$value[subject]', '$value[comment]', '$value[comment2]', '', '$value[signdate]', '$value[count]', '$value[thread]', '', '', '', '', '', '$category_1', '$category_2', '일반', '', '', '', '', 'Y', '', '', '', '', '')";
